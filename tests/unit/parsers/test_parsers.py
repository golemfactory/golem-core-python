...
def test_empty_group(demand_offer_parser, input_string, output):
...
@pytest.mark.parametrize(
    "input_string, output",
    (
        ("(& (foo=1))", ConstraintGroup([Constraint("foo", "=", "1")])),
        ("(& (float.value=1.5))", ConstraintGroup([Constraint("float.value", "=", "1.5")])),
        ("(& (foo=bar))", ConstraintGroup([Constraint("foo", "=", "bar")])),
        (
            "(& (foo=more.complex.value))",
            ConstraintGroup([Constraint("foo", "=", "more.complex.value")]),
        ),
        (
            "(& (foo=http://google.com))",
            ConstraintGroup([Constraint("foo", "=", "http://google.com")]),
        ),
        ("(& (foo=[1, 2, 3]))", ConstraintGroup([Constraint("foo", "=", ["1", "2", "3"])])),
        ("(& (foo=[a, b, c]))", ConstraintGroup([Constraint("foo", "=", ["a", "b", "c"])])),
        ("(& (some.nested.param=1))", ConstraintGroup([Constraint("some.nested.param", "=", "1")])),
        ("(& (foo<=1))", ConstraintGroup([Constraint("foo", "<=", "1")])),
        ("(& (foo>=1))", ConstraintGroup([Constraint("foo", ">=", "1")])),
    ),
)
...
def test_multiple_constraints(demand_offer_parser, input_string, output):
    result = demand_offer_parser.parse(input_string)
...
def test_nested_groups(demand_offer_parser, input_string, output):
    result = demand_offer_parser.parse(input_string)
