...
async def block_provider(func, args, e):
    activity = e.activity
    provider_id = activity.parent.parent.data.issuer_id
    bad_providers.add(provider_id)
    print(f"Blocked provider {provider_id} because of {e}")
...
async def main() -> None:
    ...
    async with golem:
        ...
        async for task_data, result in Chain(
            task_stream(),
            Map(activity_pool.execute_in_pool(execute_task), on_exception=block_provider),
            Buffer(10),
        ):
